...
1. **设置**：从仓库根目录运行 `.game.design/scripts/bash/check-prerequisites.sh --json` 并解析 FEATURE_DIR 和 AVAILABLE_DOCS。所有路径必须是绝对路径。对于参数中的单引号（如 "I'm Groot"），使用转义语法：'I'\''m Groot'（或使用双引号："I'm Groot"）。
