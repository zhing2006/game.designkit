...
2. **检查现有分支并确定编号**（仅功能型 spec）：
   ...
   b. 查找该 short-name 的最高功能编号，检查所有三个源：
      - 远程分支：`git ls-remote --heads origin | grep -E 'refs/heads/dk-[0-9]+-<short-name>$'`
      - 本地分支：`git branch | grep -E '^[* ]*dk-[0-9]+-<short-name>$'`
      - gamedesigns 目录：检查匹配 `gamedesigns/dk-[0-9]+-<short-name>` 的目录
...
2.5. **前置文档读取** （仅功能型 spec）
   ...
   b. 查看全局型规范 是否已经存在
      - 如果存在：则读取全局型规范的`spec.md`, `plan.md`, `tasks.md`文件，从中寻找最终生成的策划案文件(一般在docs目录下)
         - 如果策划案文件存在：读取并理解其内容
         - 如果策划案文件不存再：通过全局型规范内容推断世界观和设计愿景
      - 如果不存在：
        ERROR "未找到 global 型规范，建议先运行 /game.designkit.specify 创建世界观和设计愿景规范"
        结束，不再执行后续步骤

   **检查点**：在进入步骤3前，AI 必须明确回答：
     - [ ] 已读取 pillars.md
     - [ ] 已读取 dk-000-xxx/spec.md
     - [ ] 已读取 dk-000-xxx/plan.md
     - [ ] 已读取 dk-000-xxx/tasks.md
     - [ ] 已尝试读取策划案文档（如果存在）
...
3. 运行脚本创建 spec 目录和文件：
   ...
   **重要**：
   ...
   - 只能运行此脚本一次
   - JSON 输出示例：
     - 全局型：`{"BRANCH_NAME":"dk-000-game-vision","SPEC_FILE":"/path/to/gamedesigns/dk-000-game-vision/spec.md","FEATURE_NUM":"000","SPEC_TYPE":"global"}`
     - 功能型：`{"BRANCH_NAME":"dk-003-combat-system","SPEC_FILE":"/path/to/gamedesigns/dk-003-combat-system/spec.md","FEATURE_NUM":"003","SPEC_TYPE":"feature"}`
...
5. 执行以下 NLP 提取流程：
   ...
   b. 根据 SPEC_TYPE 提取关键概念：
      ...
      **功能型（feature-spec）提取内容**：
      ...
      - 功能目标（体验目标）
      - 依赖关系（依赖哪些其他 spec，如 dk-000-game-vision）
      - 玩家操作（操作流程）
      - 系统接口（与其他系统的对接点）
...
7. **规范质量验证**：写入初始规范后，根据质量标准进行验证：
   ...
   a. **创建规范质量检查清单**：在 `FEATURE_DIR/checklists/requirements.md` 生成检查清单文件，包含以下验证项：
      ...
      **Spec 类型**：全局型 / 功能型
      ...
      - [ ] Dependencies 章节明确列出依赖的 spec（至少依赖 dk-000-game-vision）
