...
while [ $i -le $# ]; do
    ...
    case "$arg" in
        ...
        --type)
            if [ $((i + 1)) -gt $# ]; then
                echo 'Error: --type requires a value (global or feature)' >&2
                exit 1
            fi
            i=$((i + 1))
            next_arg="${!i}"
            ...
            if [[ "$next_arg" != "global" ]] && [[ "$next_arg" != "feature" ]]; then
                echo 'Error: --type must be either "global" or "feature"' >&2
                exit 1
            fi
            SPEC_TYPE="$next_arg"
            ;;
...
if [ -z "$SPEC_TYPE" ]; then
    echo "Error: --type parameter is required (global or feature)" >&2
    exit 1
fi
...
find_repo_root() {
    local dir="$1"
    while [ "$dir" != "/" ]; do
        if [ -d "$dir/.git" ] || [ -d "$dir/.game.design" ]; then
            echo "$dir"
            return 0
        fi
        dir="$(dirname "$dir")"
...
# Function to get highest number from gamedesigns directory
...
if [ ${#BRANCH_NAME} -gt $MAX_BRANCH_LENGTH ]; then
    ...
    >&2 echo "[game.designkit] Truncated to: $BRANCH_NAME (${#BRANCH_NAME} bytes)"
fi

if [ "$HAS_GIT" = true ]; then
    git checkout -b "$BRANCH_NAME"
else
    >&2 echo "[game.designkit] Warning: Git repository not detected; skipped branch creation for $BRANCH_NAME"
fi

FEATURE_DIR="$GAMEDESIGNS_DIR/$BRANCH_NAME"
mkdir -p "$FEATURE_DIR"

# Select template based on spec type
if [ "$SPEC_TYPE" == "global" ]; then
    TEMPLATE="$REPO_ROOT/.game.design/templates/spec-template-global.md"
else
    TEMPLATE="$REPO_ROOT/.game.design/templates/spec-template-feature.md"
fi
...
export GAMEDESIGN_FEATURE="$BRANCH_NAME"

if $JSON_MODE; then
    printf '{"BRANCH_NAME":"%s","SPEC_FILE":"%s","FEATURE_NUM":"%s","SPEC_TYPE":"%s"}\n' "$BRANCH_NAME" "$SPEC_FILE" "$FEATURE_NUM" "$SPEC_TYPE"
else
    echo "BRANCH_NAME: $BRANCH_NAME"
    echo "SPEC_FILE: $SPEC_FILE"
    echo "FEATURE_NUM: $FEATURE_NUM"
    echo "SPEC_TYPE: $SPEC_TYPE"
    echo "GAMEDESIGN_FEATURE environment variable set to: $BRANCH_NAME"
fi
